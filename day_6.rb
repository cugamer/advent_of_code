def parse_direction(input)
  rows = input.scan(/\d+,/).map { |coord| coord.gsub(/,/, "").to_i }
  cols = input.scan(/,\d+/).map { |coord| coord.gsub(/,/, "").to_i }
  direction = rows + cols
...
def create_board
  board = Array.new
  1000.times do 
  	board << Array.new(1000, "f")
  end
  board
end

def turn_on(loc, board)
  board[loc[0]][loc[1]] = "n"
  board
end

def turn_off(loc, board)
  board[loc[0]][loc[1]] = "f"
  board
end

def toggle(loc, board)
  current = board[loc[0]][loc[1]]
  if current == "f"
    board[loc[0]][loc[1]] = "n"
  elsif current == "n"
    board[loc[0]][loc[1]] = "f"
...
def execute_switch(parsed_directions, board)
  start_row = parsed_directions[0]
  end_row = parsed_directions[1]
  start_col = parsed_directions[2]
  end_col = parsed_directions[3]
  command = parsed_directions[4]

  board[start_row..end_row].each_index do |row_index|
    board[row_index][start_col..end_col].each_index do |col_index|
      cell = [row_index + start_row, col_index + start_col]
      if command == "on"
        turn_on(cell, board)
      elsif command == "off"
        turn_off(cell, board)
